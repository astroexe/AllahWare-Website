...
$(document).ready(() => {
  const links = [
    {
      name: 'steam',
      link: 'https://steamcommunity.com/id/AllahWare',
    },
    {
      name: 'Get Synapse',
    ...
    {
      name: 'github',
      link: 'https://github.com/Allahware',
    },
  ];
  ...
  app.titleChanger(['Get', 'Good', 'Get', 'The', 'Best', 'Out', 'There,', 'Version 0.0.1']);
});
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! \nBy the way, nice to see someone from ${country} here!`], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>wanna be 1337?</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
...
setInterval(function () {
  ...
  if (aud.currentTime != vid.currentTime){
